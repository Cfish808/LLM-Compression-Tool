...
def compute_embedding_similarity(dataset1_samples, dataset2_samples, model_name="all-MiniLM-L6-v2"):
    ...
    #print(dataset1_samples, dataset2_samples)
    ...
    print("Embeddings computed for both datasets.")
...
def compute_coverage(dataset, capability, reference_datasets, tokenizer, alpha=0.6):
    ...
    print(reference_datasets[capability],"reference dataset for ",capability)
    reference_samples = get_dataset_samples(reference_datasets[capability])
    
    # ==================== 新增调试打印 ====================
    print(f"[REF DEBUG] 数据集: {reference_datasets[capability]}")
    print(f"[REF DEBUG] 共采样 {len(reference_samples)} 条")
    print(f"[REF DEBUG] 前 5 条样本（检查是否还有 None）:")
    for i, sample in enumerate(reference_samples[:5]):
        print(f"  {i+1}. {repr(sample)[:300]}...")   # repr防止换行太乱
        
    # Compute semantic similarity
    print("Computing semantic similarity...",dataset)
...
def get_dataset_samples(dataset_name, num_samples=100, seed=42):
    ...
    try:
        ...
        if dataset_name == "c4":
            dataset = datasets.load_dataset("allenai/c4",
                               data_files={'validation': 'en/c4-validation.00000-of-00008.json.gz'},
                               split='validation')
        else:
            print(f"Loading dataset: {dataset_name}")
            dataset = datasets.load_dataset(dataset_name)
            print (f"Loaded dataset {dataset_name} with splits: {dataset.keys()}")
        ...
        if text_field not in dataset.features:
            if "q_code" in dataset.features:          # code_qa 用 q_code
                text_field = "q_code"
            elif "question_stem" in dataset.features:      # tau/commonsense_qa 等用 question
                ...
                for field in dataset.features:
                    if isinstance(dataset.features[field], datasets.features.Value) and \
                       dataset.features[field].dtype == "string":
                        text_field = field
                        break
        print(f"Using text field '{text_field}' for dataset {dataset_name}")
...
def select_datasets(
    ...
    if reference_datasets is None:
        reference_datasets = {
            "commonsense": "allenai/openbookqa",
            "math": "regisss/math_qa",
