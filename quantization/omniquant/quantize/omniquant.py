...
from quantization.omniquant.models.int_llama_layer import QuantLlamaDecoderLayer
from quantization.omniquant.models.int_opt_layer import QuantOPTDecoderLayer
from quantization.omniquant.models.int_falcon_layer import QuantFalconDecoderLayer
from quantization.omniquant.quantize.int_linear import QuantLinear
...
import pdb
import gc
from .utils import let_parameters, lwc_parameters, get_omni_parameters,\
                            omni_state_dict, register_scales_and_zeros,smooth_and_quant_temporary,\
                            smooth_and_quant_inplace,clear_temp_variable,set_quant_state, NativeScalerWithGradNormCount
try:
...
def omniquant(
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, inp, **kwargs):
            inps[cache["i"]] = inp
            cache["i"] += 1
            cache["attention_mask"] = kwargs["attention_mask"][:,:,:,:args.seqlen]
            if self.is_llama:
    ...
    for i in range(len(layers)):
        ...
        if args.epochs>0:
            ...
            with torch.no_grad():
                ...
                with traincast():
                    for j in range(args.nsamples):
                        quant_inps[j] = qlayer(quant_inps[j].half().unsqueeze(0), attention_mask=attention_mask,position_ids=position_ids)[0]
