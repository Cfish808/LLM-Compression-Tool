...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        if dataset_name == 'alpaca':
            ...
            try:
                ...
                dataset = load_dataset("/mnt/usercache/zxy/EfficientQAT/EfficientQAT/data/RedPajama-Data-1T-Sample")   
            if "validation" not in dataset.keys():
                validation_split = args.eval_dataset_size
                dataset["validation"] = load_dataset(
                    "/mnt/usercache/zxy/EfficientQAT/EfficientQAT/data/RedPajama-Data-1T-Sample",
    ...
    def format_dataset(dataset, dataset_format):
        if (
            ...
            (dataset_format is None and args.name in ['alpaca', 'alpaca-clean'])
        ):
            dataset = dataset.map(extract_alpaca_dataset, remove_columns=['instruction'])
        elif dataset_format == 'oasst1' or (dataset_format is None and args.name == 'oasst1'):
            dataset = dataset.map(lambda x: {
                'input': '',
                'output': x['text'],
            })
        elif dataset_format == 'pt' or (dataset_format is None and args.name in ['c4', 'redpajama']):
    ...
    if args.name in ['c4', 'redpajama']:
        cache_dir = './cache'
        cache_dataloader = f'{cache_dir}/e2e_dataloader_{args.type}_{args.name}_{args.pt_context_len}.cache'
    ...
    elif args.name in ['deita-6k', 'deita-10k']:
        # Split train/eval for deita datasets
        raw_data = load_data(args.name)
        np.random.seed(0)
        ...
        from quantization.efficientqat.deita_dataset_train import SupervisedDataset, LazySupervisedDataset
    ...
    elif args.name == 'mix_deita_redpajama':
        cache_dir = './cache'
        cache_dataloader = f'{cache_dir}/dataloader_{args.type}_{args.name}_{args.pt_context_len}.cache'
        if os.path.exists(cache_dataloader):
            dataset = torch.load(cache_dataloader)
            print(f"load dataset from {cache_dataloader}")
        else:
            ...
            from quantization.efficientqat.deita_dataset_train import (SupervisedDataset)
        ...
        dataset = load_data(args.name)
        dataset = format_dataset(dataset, args.dataset_format)
    print(f"loading {args.name} successfully")
    
    # Split train/eval, reduce size for other datasets
    if not args.name in ['deita-6k', 'deita-10k']:
    ...
    if args.name in ['c4', 'redpajama', 'deita-6k', 'deita-10k','mix_deita_redpajama']:
