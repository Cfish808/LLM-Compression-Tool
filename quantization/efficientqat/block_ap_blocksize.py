        ...
        with torch.cuda.amp.autocast():
            for index, inps in enumerate(dataset):
                ...
                if type(layer) == torch.nn.modules.container.ModuleList:
                    xx = copy.deepcopy(inps)
                    for layer_index in range(len(layer)):
                        xx = layer[layer_index](xx, attention_mask=attention_mask,position_ids=position_ids)[0]
                    new_data = xx.to('cpu')
...
def block_ap(
    ...
    for block_index in range(0, len(layers), block_size):
        ...
        layer = layers[block_index : block_index + block_size].to(dev)
        ...
        if type(qlayer) == torch.nn.modules.container.ModuleList:
            for layer_index in range(len(qlayer)):
                set_quant_state(qlayer[layer_index],weight_quant=True)
        else: set_quant_state(qlayer,weight_quant=True)  # activate quantization
            ...
            for epoch in range(args.epochs):
                ...
                for index, (quant_inps, fp_inps) in enumerate(zip(quant_train_inps, fp_train_inps)):    
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        if type(qlayer) == torch.nn.modules.container.ModuleList:
                            xx = copy.deepcopy(input)
                            for layer_index in range(len(qlayer)):
                                xx = qlayer[layer_index](xx, attention_mask=attention_mask,position_ids=position_ids)[0]
                            quant_out = xx
                        else: quant_out = qlayer(input, attention_mask=attention_mask_batch,position_ids=position_ids)[0]
        ...
        if type(qlayer) == torch.nn.modules.container.ModuleList:
            for layer_index in range(len(qlayer)):
                set_quant_state(qlayer[layer_index],weight_quant=False)
        else: set_quant_state(qlayer,weight_quant=False)  # weight has been quantized inplace
        ...
        if type(qlayer) != torch.nn.modules.container.ModuleList: layers[block_index] = qlayer.to("cpu")
        else:
            for block_size_index in range(block_size):
                layers[block_index + block_size_index] = qlayer[block_size_index].to("cpu")
