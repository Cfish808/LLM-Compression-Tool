...
from quantization.efficientqat.int_linear_fake import QuantLinear
import torch
...
def set_weight_parameters(model, requires_grad):
    ...
    for name, m in model.named_parameters():
        if name.find('weight') > -1 and not (name.find('scale') > -1 or name.find('zero_point') > -1):
...
def set_quant_parameters(model, requires_grad):
    params = []
    for name, m in model.named_parameters():
        if name.find('scale') > -1 or name.find('zero_point') > -1:
            m.requires_grad = requires_grad
    return iter(params)  
